...
순수 JS 패키지라 어떤 OS든 Node만 있으면 동작합니다(네이티브 바이너리 없음). `@latest`를 붙인 이유는
...
검색과 `generate_wsb`는 모든 OS에서 되고, `launch_sandbox`의 자동 실행은 러너가 있는
...
연결이 안 되거나 응답이 오지 않으면 [TROUBLESHOOTING.md](TROUBLESHOOTING.md)를 참고하세요.

가장 흔한 원인은 **Claude Desktop이 `claude_desktop_config.json`과 별개로 캐시해 둔 옛 MCP 서버 등록**입니다.
...
목록에 보이는 MCP 서버를 **휴지통 아이콘으로 직접 삭제**하고(파일 수정만으로는 안 지워짐), 확장을 모두 제거한
...
- npm에 순수 JS 패키지 `tablecloth-mcp`를 올립니다(네이티브 바이너리 없음).
...
레인별로 두 구현이 있고, 둘 다 [`shared/`](shared/)를 단일 진실 원천으로 소비합니다([SPEC.md](SPEC.md)).

- **`.mcpb` + npm 레인**: [`node/`](node/) 의 Node/TS 구현(순수 JS). `cd node && npm ci && npm run build`.
- **NuGet 도구(dnx) 레인**: 이 리포 루트의 C# 구현. `dotnet build -c Release`.

두 구현의 동등성은 conformance 하네스로 검증합니다: `cd node && npm run conformance`
(리포 루트에서 `dotnet build -c Release`, `node`에서 `npm run build` 선행). CI가 매 빌드에서 실행합니다.
